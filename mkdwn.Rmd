...
The cleaned data set now contains 54 variables (54 columns) and in order to determine which ones are actual useful predictors we use two strategies: using the nearZeroVar() function to identify and remove variables with low variability and principal component analysis (PCA) to identify correlation between the remaining variables and reduce the dimensional of the problem.
...
```
The new data set now consists of only 8 variables. For the training we use a random forest algorithm and to determine the expected out of sample accuracy we devide the original 19622 training entries into a training and validation set after fit a the random forest model and compute the accuracy of the fit on the validation and training sets. This is the done with the following code:
...
We see that the accuracy on the validation set with this approach is 92.7% percent while the accuracy for the training set is 100%.
...
We see that the predictions disagree only in two cases: for sample 6 and 11. Using these predicions we get 19 out of the 20 predictions correct. When submitting the predictions in the project submission the only incorrect prediction is for number 7.
